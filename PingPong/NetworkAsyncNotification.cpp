#include "NetworkAsyncNotification.h"

NetworkAsyncNotification::NetworkAsyncNotification() {
...
void NetworkAsyncNotification::HandleEvent(Ball& ball) {
...
int NetworkAsyncNotification::EventAccept(int sigEventIdx)
{
...
int NetworkAsyncNotification::EventRead(int sigEventIdx, Ball& ball)
{
...
int NetworkAsyncNotification::EventClose(int sigEventIdx)
{
...
void NetworkAsyncNotification::CompressSockets(SOCKET hSockArr[], int idx, int total)
{
	for (int i = idx; i < total; i++)
		hSockArr[i] = hSockArr[i + 1];
}
void NetworkAsyncNotification::CompressEvents(WSAEVENT hEventArr[], int idx, int total)
{
	for (int i = idx; i < total; i++)
		hEventArr[i] = hEventArr[i + 1];
}
void NetworkAsyncNotification::ErrorHandling(const char* buf)
{
	std::cerr << buf << std::endl;
	exit(EXIT_FAILURE);
}

int NetworkAsyncNotification::GetPosY1()
{
	return pos[1];
}

int NetworkAsyncNotification::GetPosY2()
{
	return pos[2];
}